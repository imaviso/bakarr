...
- **Alert Red** (`oklch(0.577 0.245 27.325)`): The only color with meaningful chroma. Reserved for errors, destructive actions, and invalid states. Used sparingly.

### Named Rules

**The Zero Chroma Rule.** Every neutral token must have chroma ≤ 0.01. If a color looks tinted, it is wrong. The artwork provides all hue.

**The One Voice Rule.** Alert Red is the only saturated color in the system. Its rarity is the point. Never introduce a second accent hue.
