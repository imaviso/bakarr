...
| 4   | **done** — all production `make*` orchestration bags converted to `Effect.Service`; pure-leaf `dependencies:` filled on every applicable Service. Stateful singletons (EventBus, OperationsProgress, RuntimeConfigSnapshotService, BackgroundWorkerMonitor, OperationsTaskLauncherService) and platform-provided services (FileSystem, MediaProbe, TorrentClientService, ExternalCall, metadata/identity clients, config) stay lifecycle-provided: services `yield*` them from context and intentionally do not list them in `dependencies:` to avoid duplicate stateful instances. `ExternalCall` remains a `Context.Tag` because `Effect.Service` leaks construction `R` into generic method types (`tryExternal`/`tryExternalEffect`). |
| 5   | **done** — PureDbLeaves once at lifecycle; media/ops take leaves arg; anime* → media* wiring names                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 6   | **done** — Download-only SQL (status stats aggregate, recent event rows, event presentation contexts) moved from SystemStatsRepository to DownloadRepository Tag; count\*Downloads test helpers in src/test/; loadSystemLibraryStatsAggregate stays as dashboard read-model (cross-aggregate by design)                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| 10  | **done** — job edges map to Infrastructure\|Database\|WorkerTimeout; HTTP domain unions intentional (ADR-0002)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
