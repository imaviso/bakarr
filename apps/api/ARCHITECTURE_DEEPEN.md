...
| 4   | **partial** — stateless deps filled on 8 no-deps Services (SystemConfigService, QualityProfileService, ReleaseProfileService, StreamTokenSigner, OperationsTaskLauncherService, RuntimeConfigSnapshotService, AuthCredentialService, SystemLogService) + BackgroundJobStatusService (SystemStatsRepository only); stateful singleton `.Default`s (EventBus, OperationsProgress, RuntimeConfigSnapshotService, BackgroundWorkerMonitor, OperationsTaskLauncherService) NOT added to partial-deps — would create duplicate instances; full Default trees still blocked by client/config/platform nests |
