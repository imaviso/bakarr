...
it("deriveEpisodeTimelineMetadata classifies aired, future, and unknown episodes", () => {
...
it("summarizeEpisodeCoverage prefers air date and formats sorted unique episodes", () => {
  assert.deepStrictEqual(
    summarizeEpisodeCoverage({ airDate: "2025-05-01", episodeNumbers: [3, 2, 2] }),
    "Air date 2025-05-01",
  );
  assert.deepStrictEqual(
    summarizeEpisodeCoverage({ episodeNumbers: [3, 1, 2, 2] }),
    "Episodes 1-3",
  );
  assert.deepStrictEqual(
    summarizeEpisodeCoverage({ episodeNumbers: [5, 1, 3] }),
    "Episodes 1, 3, 5",
  );
  assert.deepStrictEqual(
    summarizeEpisodeCoverage({ episodeNumbers: [1, 0, Number.NaN] }),
    undefined,
  );
});
