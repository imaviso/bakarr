...
export function deriveEpisodeTimelineMetadata(
  aired?: string,
  now?: Date,
): Pick<MediaUnit, "airing_status" | "is_future"> {
  if (!aired) {
...
export function summarizeEpisodeCoverage(input: {
  ...
  const isContiguous = unitNumbers.every((value, index) => {
    if (index === 0) {
      return true;
    }

    const previous = unitNumbers[index - 1];
    return previous !== undefined && value === previous + 1;
  });

  if (isContiguous) {
    return `Episodes ${unitNumbers[0]}-${unitNumbers[unitNumbers.length - 1]}`;
  }

  return `Episodes ${unitNumbers.join(", ")}`;
}
...
export function inferAiredAt(
  ...
  const inferredFromSchedule = inferFromNearestScheduledEpisode(unitNumber, futureAiringSchedule);
...
export function scoreAnimeSearchResultMatch(
  parsedTitle: string,
  candidate: Pick<MediaSearchResult, "title" | "synonyms">,
) {
