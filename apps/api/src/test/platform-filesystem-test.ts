...
export function platformFsMakeTempDir(
  options?: PlatformFileSystem.MakeTempDirectoryOptions,
) {
  return runPlatformFs(
    Effect.flatMap(
      PlatformFileSystem.FileSystem,
      (fs) => fs.makeTempDirectory(options),
    ),
  );
}

export function platformFsMakeTempFile(
  options?: PlatformFileSystem.MakeTempFileOptions,
) {
  return runPlatformFs(
    Effect.flatMap(
      PlatformFileSystem.FileSystem,
      (fs) => fs.makeTempFile(options),
    ),
  );
}

export function platformFsRemovePath(
  path: string,
  options?: PlatformFileSystem.RemoveOptions,
) {
  return runPlatformFs(
    Effect.flatMap(
      PlatformFileSystem.FileSystem,
      (fs) => fs.remove(path, options),
    ),
  );
}
...
export function platformFsMkdirPath(
  ...
  return runPlatformFs(
    Effect.flatMap(
      PlatformFileSystem.FileSystem,
      (fs) => fs.makeDirectory(path, options),
    ),
  );
}
...
export function platformFsWriteTextFile(path: string, data: string) {
  return runPlatformFs(
    Effect.flatMap(
      PlatformFileSystem.FileSystem,
      (fs) => fs.writeFileString(path, data),
    ),
  );
}
...
export function platformFsStatPath(path: string) {
  return runPlatformFs(
    Effect.flatMap(PlatformFileSystem.FileSystem, (fs) => fs.stat(path)),
  );
}
