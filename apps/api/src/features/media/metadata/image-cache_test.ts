...
it.scoped("cacheAnimeMetadataImages uses provided HttpClient for remote images", () =>
  withFileSystemSandboxEffect(({ fs, root }) =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(result.coverImage, "/api/images/media/55/cover.png");
      assert.deepStrictEqual(yield* exists(fs, `${root}/media/55/cover.png`), true);
...
it.scoped("cacheAnimeMetadataImages saves cover and banner files locally", () =>
  withFileSystemSandboxEffect(({ fs, root }) =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(result.coverImage, "/api/images/media/99/cover.png");
      assert.deepStrictEqual(result.bannerImage, "/api/images/media/99/banner.png");
      assert.deepStrictEqual(yield* exists(fs, `${root}/media/99/cover.png`), true);
      assert.deepStrictEqual(yield* exists(fs, `${root}/media/99/banner.png`), true);
    }),
  ),
);
...
it.scoped("cacheAnimeMetadataImages fails on unsupported image types", () =>
  withFileSystemSandboxEffect(({ fs, root }) =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(yield* exists(fs, `${root}/media/77/cover.png`), false);
      assert.deepStrictEqual(yield* exists(fs, `${root}/media/77/banner.png`), false);
    }),
  ),
);
