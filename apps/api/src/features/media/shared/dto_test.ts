...
it.effect("toMediaDto builds progress, metadata, and decoded arrays", () =>
  Effect.gen(function* () {
    const row = {
      ...
      bannerImage: "/api/images/media/20/banner.jpg",
      coverImage: "/api/images/media/20/cover.jpg",
      ...
      recommendedMedia: '[{"id":30,"title":{"english":"Boruto","romaji":"Boruto"}}]',
    ...
    ] satisfies Array<typeof mediaUnits.$inferSelect>;

    const result = yield* toMediaDto(
      row,
      deriveDetailProgress(episodeRows, row.unitCount ?? undefined),
    );
    ...
    assert.deepStrictEqual(result.progress.latest_downloaded_unit, 3);
    assert.deepStrictEqual(result.progress.total, 4);
    assert.deepStrictEqual(result.progress.missing, [2, 4]);
    assert.deepStrictEqual(result.progress.next_missing_unit, 2);
    ...
    assert.deepStrictEqual(result.banner_image, "/api/images/media/20/banner.jpg");
    assert.deepStrictEqual(result.related_media?.[0]?.relation_type, "PREQUEL");
    assert.deepStrictEqual(result.recommended_media?.[0]?.title.english, "Boruto");
...
it.effect("toMediaDto handles media with unknown episode totals", () =>
  Effect.gen(function* () {
    const result = yield* toMediaDto(
      ...
      deriveDetailProgress([], undefined),
    );
    ...
    assert.deepStrictEqual(result.progress.latest_downloaded_unit, undefined);
    assert.deepStrictEqual(result.progress.next_missing_unit, undefined);
