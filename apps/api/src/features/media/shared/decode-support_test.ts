...
it("decodeStoredStringListEffect fails on corrupt JSON", () =>
  Effect.gen(function* () {
    ...
    assertStoredDataError(exit, "Stored media tags JSON is corrupt");
  }));
...
it("decodeStoredNumberListEffect fails on non-number array", () =>
  Effect.gen(function* () {
    ...
    assertStoredDataError(exit, "Stored media mediaUnits JSON is corrupt");
  }));
...
it("decodeStoredSynonymsEffect fails on invalid JSON", () =>
  Effect.gen(function* () {
    ...
    assertStoredDataError(exit, "Stored media synonyms JSON is corrupt");
  }));
