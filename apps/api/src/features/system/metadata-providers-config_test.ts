...
it("defaults Tenrai requests per minute when absent", () =>
  Effect.gen(function* () {
    const normalized = yield* normalizeMetadataProvidersConfig({
      anidb: {
        client: "bakarr",
    ...
    assert.deepStrictEqual(normalized.tenrai.requests_per_minute, 60);
  }));

it("rejects Tenrai requests per minute outside 1-120 instead of coercing", () =>
  Effect.gen(function* () {
    for (const requestsPerMinute of [0, -5, 121, 1.5]) {
      const exit = yield* Effect.exit(
        normalizeMetadataProvidersConfig({
          anidb: {
            client: "bakarr",
          ...
          tenrai: { requests_per_minute: requestsPerMinute },
        }),
      );
