...
const insertAnime = Effect.fn("Test.insertAnime")(function* (
  ...
  yield* Effect.tryPromise(() => db.insert(schema.anime).values({
    ...
    run: (db) =>
      withFileSystemSandboxEffect(({ fs, root }) =>
        Effect.gen(function* () {
          ...
          const result = yield* resolveEpisodeFileEffect({ animeId: 1, db: appDb, episodeNumber: 1, fs });
...
it.scoped("resolveEpisodeFileEffect returns root inaccessible state when anime root is inaccessible", () =>
  withSqliteTestDbEffect({
    migrationsFolder: DRIZZLE_MIGRATIONS_FOLDER,
    run: (db) =>
      withFileSystemSandboxEffect(({ fs, root }) =>
...
it.scoped("resolveEpisodeFileEffect returns outside-root state when mapping escapes anime root", () =>
  withSqliteTestDbEffect({
    migrationsFolder: DRIZZLE_MIGRATIONS_FOLDER,
    run: (db) =>
      withFileSystemSandboxEffect(({ fs, root }) =>
