...
export const loadQualityProfile = Effect.fn(
  "ProfileRepository.loadQualityProfile",
)(function* (
  db: AppDatabase,
  name: string,
) {
  const rows = yield* tryDatabasePromise(
    ...
    () =>
      db.select().from(qualityProfiles).where(
...
export const loadReleaseRules = Effect.fn(
  "ProfileRepository.loadReleaseRules",
)(function* (
  db: AppDatabase,
  animeRow: { releaseProfileIds: string },
) {
  const assignedIds = yield* effectDecodeNumberList(
    animeRow.releaseProfileIds,
  ).pipe(mapDecodeError("Failed to load release rules"));

  const rows = yield* tryDatabasePromise(
    "Failed to load release rules",
