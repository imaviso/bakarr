    ...
    const runRssCheckBase = Effect.fn("OperationsService.runRssCheckBase")(function* () {
      ...
      return yield* Effect.gen(function* () {
        ...
        const processRssFeed = Effect.fn("operations.rss.feed")(function* (
          ...
          const items = yield* rssRunnerService.fetchItems(feed.url).pipe(
          ...
          const profile = yield* qualityProfileService.requireQualityProfile(animeRow.profileName);
        ...
        Effect.catchTag("OperationsAnimeNotFoundError", (error) =>
          markJobFailed(db, "rss", error, nowIso).pipe(
            Effect.zipRight(
              Effect.fail(
                new OperationsInfrastructureError({
