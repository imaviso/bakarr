...
export function makeUnmappedControlWorkflow(input: {
  db: AppDatabase;
  fs: FileSystemShape;
  matchAndPersistUnmappedFolder: UnmappedScanWorkflowShape["matchAndPersistUnmappedFolder"];
  nowIso: () => Effect.Effect<string>;
  tryDatabasePromise: TryDatabasePromise;
}) {
  const { db, fs, matchAndPersistUnmappedFolder, nowIso, tryDatabasePromise } = input;
