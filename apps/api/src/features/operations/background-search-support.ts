...
export function makeBackgroundSearchSupport(input: {
  ...
  const queueReleaseIfEligible = Effect.fn("OperationsService.queueReleaseIfEligible")(
    function* (input: {
      ...
      const queueEffect = Effect.gen(function* () {
        const overlapping = yield* tryDatabasePromise(input.contextMessage, () =>
          hasOverlappingDownload(
            db,
            input.animeRow.id,
            input.item.infoHash,
  ...
  const triggerSearchMissingRaw = Effect.fn("OperationsService.triggerSearchMissing")(function* (
    animeId?: number,
  ) {
