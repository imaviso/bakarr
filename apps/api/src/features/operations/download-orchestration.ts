...
import {
  makeDownloadReconciliationService,
} from "./download-reconciliation-service.ts";
import {
  makeDownloadTorrentLifecycleService,
} from "./download-torrent-lifecycle-service.ts";
import { makeDownloadTriggerService } from "./download-trigger-service.ts";
import {
  type DownloadOrchestrationInput,
  mapQBitState,
} from "./download-orchestration-shared.ts";
...
export function makeDownloadOrchestration(input: DownloadOrchestrationInput) {
  ...
  const triggerService = makeDownloadTriggerService({
    ...
    syncDownloadsWithQBitEffect:
      torrentLifecycleService.syncDownloadsWithQBitEffect,
  });
        ...
        yield* torrentLifecycleService.syncDownloadsWithQBitEffect().pipe(
          Effect.catchAll((error) =>
            error instanceof DatabaseError ? Effect.fail(error) : Effect.fail(
              input.dbError("Failed to sync downloads with qBittorrent")(error),
            )
          ),
        );
  ...
  return {
    applyDownloadActionEffect:
      torrentLifecycleService.applyDownloadActionEffect,
    getDownloadProgressSnapshotEffect:
      triggerService.getDownloadProgressSnapshotEffect,
    maybeCleanupImportedTorrent:
      reconciliationService.maybeCleanupImportedTorrent,
    publishDownloadProgress: triggerService.publishDownloadProgress,
    reconcileCompletedTorrentEffect:
      reconciliationService.reconcileCompletedTorrentEffect,
    reconcileDownloadByIdEffect:
      reconciliationService.reconcileDownloadByIdEffect,
