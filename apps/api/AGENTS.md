...
- `src/features/` contains domain services, orchestration, and live layers; grouped into vertical clusters (e.g. `media/add/`, `media/metadata/`, `operations/download/`, `operations/search/`). Keep `errors.ts` and `layer.ts` at feature root. `src/http/` keeps route adapters thin; `src/infra/` holds shared Effect utilities; `src/db/` owns schema and persistence.
- Follow `EFFECT_GUIDE.md`: prefer `Effect.gen`, `Effect.fn`, `Context.Service`, `Layer.*`, and `Schema`-first modeling over compatibility layers or manual dependency bags.
