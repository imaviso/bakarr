...
- **Shared atoms:** `src/components/shared/{field-error,stat-dot,meta-text}.tsx` for repeated micro-patterns — prefer an explicit prop over a magic class string. Use `FieldError` for all form field errors and `ConfirmDialog` for confirmations instead of hand-rolling AlertDialogs. Do not mass-migrate existing `text-xs text-muted-foreground` spans onto `MetaText` (most carry extra classes/element roles); reach for it in new code where the plain pattern fits.
