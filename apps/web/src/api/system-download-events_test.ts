...
describe("system download events API helpers", () => {
  it("builds export URLs with all meaningful filters", () => {
    ...
    expect(url).toBe(
      "/api/downloads/events/export?media_id=0&download_id=12&event_type=status_changed&status=completed&start_date=2026-01-01+00%3A00%3A00&end_date=2026-02-01+23%3A59%3A59&limit=500&order=asc&format=csv",
    );
  });

  it("omits empty optional filters but keeps explicit numeric values", () => {
    const url = getDownloadEventsExportUrl({ mediaId: 0, eventType: "", limit: 0, status: "" });

    expect(url).toBe("/api/downloads/events/export?media_id=0&limit=0&format=json");
  });
