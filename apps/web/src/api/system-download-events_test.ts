...
describe("system download events API helpers", () => {
  it("builds export URLs with all meaningful filters", () => {
    ...
    expect(url).toBe(
      "/api/downloads/events/export?media_id=0&download_id=12&event_type=status_changed&status=completed&start_date=2026-01-01&end_date=2026-02-01&limit=500&order=asc&format=csv",
    );
  });
