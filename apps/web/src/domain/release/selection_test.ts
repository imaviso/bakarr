...
import { getReleaseConfidence } from "./selection";
...
it("getReleaseConfidence prefers SeaDex best over other hints", () => {
  ...
  if (confidence?.label !== "High confidence" || confidence.reason !== "SeaDex Best release") {
    throw new Error(`Unexpected confidence metadata: ${JSON.stringify(confidence)}`);
  }
});
...
it("getReleaseConfidence flags remakes for review", () => {
  ...
  if (confidence?.label !== "Review" || confidence.reason !== "Marked as remake") {
    throw new Error(`Expected remake review metadata, got ${JSON.stringify(confidence)}`);
  }
});
