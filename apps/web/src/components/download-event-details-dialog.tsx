...
export function DownloadEventDetailsDialog(
  props: DownloadEventDetailsDialogProps,
) {
  const summary = () =>
  ...
  return (
    ...
    >
      <DialogContent class="max-w-2xl">
        <DialogHeader>
          <DialogTitle>
            {props.event?.anime_title ?? props.event?.torrent_name ??
            ...
            {props.event?.download_id !== undefined
              ? (
                <Badge variant="outline">
                  Download #{props.event.download_id}
                </Badge>
              )
              : null}
            {summary()?.coverage
