...
function chartKeyName(value: unknown): string | undefined {
  return typeof value === "string" || typeof value === "number" ? String(value) : undefined;
}

type CSSVarStyle = React.CSSProperties & Record<`--${string}`, string | undefined>;

function cssVars(vars: Record<`--${string}`, string | undefined>): CSSVarStyle {
  return vars;
}
...
function ChartTooltipContent({
  ...
  const tooltipLabel = React.useMemo(() => {
    ...
    const [item] = payload;
    const key = labelKey ?? chartKeyName(item?.dataKey) ?? chartKeyName(item?.name) ?? "value";
    ...
    >
      ...
      <div className="grid gap-1.5">
        {payload
          ...
          .map((item, index) => {
            const key = nameKey ?? chartKeyName(item.name) ?? chartKeyName(item.dataKey) ?? "value";
            const itemConfig = getPayloadConfigFromPayload(config, item, key);
  ...
  return (
    ...
    >
      {payload
        .filter((item) => item.type !== "none")
        .map((item) => {
          const key = nameKey ?? chartKeyName(item.dataKey) ?? "value";
          const itemConfig = getPayloadConfigFromPayload(config, item, key);
