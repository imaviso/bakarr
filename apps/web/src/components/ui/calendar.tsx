        ...
        <div key={i} className="flex w-full flex-col gap-4">
          <div className="flex h-(--cell-size) w-full items-center justify-center gap-1 px-(--cell-size)">
            {captionLayout === "dropdown" ? (
              <>
                <MonthDropdown {...(headerFormat ? { format: headerFormat } : {})} />
                <YearDropdown {...(headerFormat ? { format: headerFormat } : {})} />
              </>
            ) : (
              <CalendarHeading
                offset={{ months: i }}
                {...(headerFormat
                  ? {
                      format: {
                        ...(headerFormat.day ? { day: headerFormat.day } : {}),
                        ...(headerFormat.month ? { month: headerFormat.month } : {}),
                        ...(headerFormat.year ? { year: headerFormat.year } : {}),
                        ...(headerFormat.era ? { era: headerFormat.era } : {}),
                      },
                    }
                  : {})}
...
function MonthDropdown({ format }: { format?: Intl.DateTimeFormatOptions }) {
  return (
    <CalendarMonthPicker {...(format?.month ? { format: format.month } : {})}>
      {(props) => (
...
function YearDropdown({ format }: { format?: Intl.DateTimeFormatOptions }) {
  const yearFormat =
    format?.year || format?.era
      ? {
          ...(format?.year ? { year: format.year } : {}),
          ...(format?.era ? { era: format.era } : {}),
        }
      : undefined;
  return (
    <CalendarYearPicker {...(yearFormat ? { format: yearFormat } : {})}>
      {(props) => (
        <Select {...props} className="relative">
          <SelectTrigger>
            <SelectValue />
