...
function DialogOverlay({
  ...
  return (
    <ModalOverlayPrimitive
      ...
      className={cn(
        "fixed inset-0 isolate z-50 bg-scrim/10 duration-100 data-entering:animate-in data-entering:fade-in-0 data-exiting:animate-out data-exiting:fade-out-0 supports-backdrop-filter:backdrop-blur-xs",
        className,
      )}
      {...props}
    >
  ...
  return (
    <DialogOverlay isDismissable={isDismissable} {...props}>
      ...
      >
        <DialogPrimitive
          data-slot="dialog"
          className="[display:inherit] [gap:inherit] [flex-direction:inherit] flex-1 min-h-0 w-full outline-none"
        >
          {children}
          {showCloseButton && (
