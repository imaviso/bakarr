  ...
  return (
    <Card className="overflow-hidden flex flex-col transition-colors hover:border-primary/50 group">
      <div className="relative aspect-[2/3] w-full bg-muted overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-4 md:opacity-0 md:group-hover:opacity-100 md:group-focus-within:opacity-100">
          <Button
            size="sm"
      ...
      <CardContent className={cn("p-4 flex-1", props.compact ? "p-3" : "p-4")}>
        ...
        {!props.compact && <AltTitlesSubtitle anime={props.media} />}
        <div className="flex flex-wrap gap-1.5 mt-auto">
          ...
          {props.showSearchMeta && formatMatchConfidence(props.media.match_confidence) && (
            <Badge
              variant="outline"
          ...
          {props.media.status && (
            <Badge
              ...
              className={cn(
                ...
                props.media.status?.toLowerCase() === "releasing"
                  ? "text-success border-success/30"
                  : "text-muted-foreground",
              )}
            >
              {props.media.status?.replace("_", " ").toLowerCase()}
            </Badge>
          )}
