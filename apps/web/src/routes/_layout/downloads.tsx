		...
		<TableRow class="group h-12">
			...
			<TableCell class="text-right">
				<div class="flex items-center justify-end gap-1 opacity-0 transition-opacity group-hover:opacity-100">
					<Show when={props.item.state.toLowerCase().includes("paused") || props.item.state.toLowerCase().includes("queued") || props.item.state.toLowerCase().includes("error")}
						fallback={
							<Button variant="ghost" size="icon" class="h-7 w-7" onClick={handlePause} disabled={!props.item.id || pauseDownload.isPending}>
								<IconPlayerPause class="h-4 w-4" />
							</Button>
						}
					>
						<Button variant="ghost" size="icon" class="h-7 w-7" onClick={handleResume} disabled={!props.item.id || resumeDownload.isPending}>
							<IconPlayerPlay class="h-4 w-4" />
						</Button>
					</Show>
					<Show when={props.item.state.toLowerCase().includes("error")}>
						<Button variant="ghost" size="icon" class="h-7 w-7" onClick={handleRetry} disabled={!props.item.id || retryDownload.isPending}>
...
function DownloadRow(props: { item: Download; isHistory?: boolean }) {
	...
	return (
		<TableRow class="group h-12">
			...
			<TableCell class="text-right">
				<div class="flex items-center justify-end gap-1 opacity-0 transition-opacity group-hover:opacity-100">
					<Show when={props.item.status?.toLowerCase() === "completed" && !props.item.reconciled_at}>
						<Button variant="ghost" size="icon" class="h-7 w-7" onClick={handleReconcile} disabled={reconcileDownload.isPending}>
							<IconCheck class="h-4 w-4" />
						</Button>
					</Show>
					<Show when={props.item.status?.toLowerCase() === "failed" || props.item.status?.toLowerCase() === "error"}>
						<Button variant="ghost" size="icon" class="h-7 w-7" onClick={handleRetry} disabled={retryDownload.isPending}>
							<IconRefresh class="h-4 w-4" />
						</Button>
					</Show>
					<Button variant="ghost" size="icon" class="h-7 w-7" onClick={handleDelete} disabled={deleteDownload.isPending}>
