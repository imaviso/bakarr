		...
		<div class="space-y-6">
					...
					>
						{(dashboard) => (
							<>
								...
								<DashboardMetricCard label="Failed" value={dashboard().failed_downloads} highlight="text-red-500" />
								<DashboardMetricCard label="Imported" value={dashboard().imported_downloads} highlight="text-green-500" />
								<DashboardMetricCard label="Running Jobs" value={dashboard().running_jobs} highlight="text-blue-500" />
							</>
						)}
					</Show>
				</div>
			...
			<Card class="border-dashed">
				...
				<div class="p-4 space-y-3">
					<Show when={dashboardQuery.data?.recent_download_events?.length} fallback={<div class="text-sm text-muted-foreground">No recent download events</div>}>
						<For each={dashboardQuery.data?.recent_download_events ?? []}>{(event) => <DownloadEventRow event={event} />}</For>
					</Show>
				</div>
			</Card>
	...
	return (
		<div class="rounded-lg border border-border/60 bg-card p-3 space-y-2">
			<div class="flex items-center justify-between gap-2">
				...
				<Badge variant="outline" class={cn(props.job.is_running && "border-blue-500/40 text-blue-600")}>
	...
	return (
		<div class="rounded-lg border border-border/60 bg-card p-3 space-y-1">
			<div class="flex items-center justify-between gap-2">
				...
				<div class="text-xs text-muted-foreground">{formatLogTimestamp(props.event.created_at)}</div>
			</div>
