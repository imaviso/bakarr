...
function PathMappingsEditor(props: {
	value: string[][];
	onChange: (value: string[][]) => void;
	placeholder?: string;
	rows?: number;
	...
	return (
		...
		>
			...
			<Show when={showsGeneral()}>
				...
				<form.Field name="library.recycle_cleanup_days">
					{(field) => (
						<SettingRow
			...
			<Show when={showsAutomation()}>
				...
				<form.Field name="nyaa.default_category">
					{(field) => (
						<SettingRow
				...
				<form.Field name="nyaa.preferred_resolution">
					{(field) => (
						<SettingRow
			...
			<Show when={showsAutomation()}>
				...
				<form.Field name="downloads.prefer_dual_audio">
					{(field) => (
						<SettingRow
				...
				<form.Field name="downloads.preferred_codec">
					{(field) => (
						<SettingRow
							label="Preferred Codec"
							description="Optional codec preference for ranking"
						>
							<Input
								value={field().state.value ?? ""}
								onInput={(e) => field().handleChange(e.currentTarget.value)}
				...
				<form.Field name="downloads.preferred_groups">
					{(field) => (
						<SettingRow
				...
				<form.Field name="downloads.remote_path_mappings">
					{(field) => (
						<SettingRow
						...
						>
							<div class="w-80 space-y-2">
								...
								/>
								<div class="text-[10px] text-muted-foreground">
									Used when qBittorrent reports a different path than Bakarr can
									see locally.
								</div>
							</div>
						</SettingRow>
				...
				<form.Field name="downloads.reconcile_completed_downloads">
					{(field) => (
						<SettingRow
				...
				<form.Field name="downloads.remove_torrent_on_import">
					{(field) => (
						<SettingRow
				...
				<form.Field name="downloads.delete_download_files_after_import">
					{(field) => (
						<SettingRow
