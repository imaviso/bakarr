{
  ...
  # Fixed-output derivation: download packages without running scripts.
  # Hash is deterministic across machines (same lockfile → same hash).
  pnpmDeps = stdenvNoCC.mkDerivation {
    pname = "bakarr-pnpm-deps";
    version = "0.1.0";
    inherit src;
    ...
    buildPhase = ''
      runHook preBuild
      pnpm fetch --store-dir $out --frozen-lockfile --ignore-scripts
      runHook postBuild
    '';
    ...
    outputHash = "sha256-hAvkigG4MczQdcYqxbiZt3RdTxyyPUxVLBzHuTZt0aM=";
  };
in
